...
// Пробуем .png если .jpg не найден, иначе показываем эмодзи
document.querySelectorAll('.gallery-photo').forEach(img => {
  img.addEventListener('error', function onJpgError() {
    ...
    if (src && src.endsWith('.jpg')) {
      // Снимаем этот слушатель чтобы не зациклиться
      this.removeEventListener('error', onJpgError);
      // Вешаем финальный фолбэк на .png
      this.addEventListener('error', function onPngError() {
        this.removeEventListener('error', onPngError);
      ...
      // Уже .png и тоже не загрузилось — показываем эмодзи
      this.style.display = 'none';
      const fallback = this.nextElementSibling;
      if (fallback) fallback.style.display = 'flex';
    }
