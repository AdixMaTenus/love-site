...
// ImageResponse будет загружен динамически внутри хэндлера если нужен
...
export default async function handler(req, res) {
  try {
    ...
    const useVercelOg = process.env.USE_VERCEL_OG === 'true';

    // Если разрешено использовать @vercel/og, попытаемся загрузить и использовать
    if (useVercelOg) {
      try {
